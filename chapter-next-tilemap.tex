...
The Tilemap layer is closely tied with ULA. Memory wise, it always exists in 16K bank 5. By default, this page is loaded into 16K slot 1 \MemAddr{4000}-\MemAddr{7FFF} (examples here will assume this configuration, if you load into a different slot, you will have to adjust addresses accordingly).
...
As of core 3.1.8 tilemap and tile definitions may appear in the first 8K of 16K bank 7 (the ``shadow screen'' memory which not paged in by default).
This is enabled by setting bit 7 of registers \MemAddr{6E} and \MemAddr{6F}.
Because only 8K is available within bank 7 for this purpose addresses wrap across 8K boundaries.
...
\begin{NextPort}
	...
	\PortBits{3-0}
		\PortDesc{Index of transparent colour into tilemap palette, 0xF after reset}
\end{NextPort}
