...
Before we can use sprites, we need to load their data into FPGA memory. This example introduces a generic routine that uses DMA to copy from given memory to FPGA. Don't worry if it seems like magic - it's implemented as a reusable routine, just copy it to your project. Routine requires 3 parameters:
...
See section \XRef{zx_next_dma} for more details on how to use the DMA feature.
